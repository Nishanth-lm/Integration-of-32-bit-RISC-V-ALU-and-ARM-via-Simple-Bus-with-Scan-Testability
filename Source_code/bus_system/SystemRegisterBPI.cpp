...
{
  // Gets Information from the payload
  tlm::tlm_command cmd = trans.get_command();
  unsigned int   adr = trans.get_address() / BYTESPERMEMLINE;
  ...
  if (adr >= (RegisterSize/BYTESPERMEMLINE))
  {
...
/*-----------------------------------------------------------------------------
-- void darThread(void): Data read; data comes from the SlaveFunc and triggers the bus transport.
-----------------------------------------------------------------------------*/
template<class T>
void SystemRegisterBPI<T>::darThread(void)
{
  ...
  while(true)
  {
    ...
    // give b_transport a note, that the data is now available and can be placed on the bus
