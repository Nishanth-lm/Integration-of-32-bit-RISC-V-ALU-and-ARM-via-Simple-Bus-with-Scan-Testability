...
#include "scanchain/scanchaintop.cpp"
  ...
  //, sysbus2("System-Bus2")
{
  cout << left << setw(30) << "ConstructorCPP - top:         "
   ...
   if(TESTMODE == 0){
  // Construct the ARM
  arm    = new ARMTop<T>("ARM", 1);

   // Construct the ALU
  alu    = new ALUtop<T>("ALU_top",2);
   }
  else{
   // Construct the scanchain
  scanchain    = new scanchainTop<T>("scanchain", 10);}
   ...
   if(TESTMODE == 0)
 {
  ...
  arm->mbpi->Master1Socket(sysbus.slaveSocket[0]);}
   ...
   scanchain->mbpi->Master1Socket(sysbus.slaveSocket[0]); 
  }
  ...
  if(TESTMODE == 0)
 {
  alu->data_op1_s(gpio0_o_s);
  alu->data_op2_s(gpio1_o_s);
  alu->op_code_s (gpio2_o_s);
  alu->result_s(gpio3_i_s);
  alu->flag_s(gpio4_i_s);
 }
